...
namespace Syncfusion.Maui.Charts
{
    ...
    public abstract class CircularSeries : ChartSeries
    {
        ...
        ///     
        ///     series.DataLabelSettings = new CircularDataLabelSettings(){ LabelPlacement = Placement.OutSide };
        /// ]]></code>
        /// ***
        /// </example>
        ...
        internal override void UpdateLegendIconColor()
        {
            ...
            {
                foreach (CircularLegendItem legendItem in legendItems)
                {
                    if (legendItem != null)
                    {
                        legendItem.IconBrush = legendItem.Index == SelectedIndex && SelectionBrush != null ? SelectionBrush : GetFillColor(legendItem.Index) ?? new SolidColorBrush(Colors.Transparent);
                    }
                }
            }
        }
