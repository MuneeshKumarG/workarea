        ...
        private void MeasureRectBasedOnOrigin(ChartAxis content, List<double> nearSizes, List<double> farSizes, ref bool isContinue, bool isFirstLayout)
        {
            ...
            if ((content.Origin != 0 && isFirstLayout) || 0 < value && 1 > value)
            {
                if (content.HeaderPosition == AxisHeaderPosition.Far)
                {
                    ...
                    double position = isOpposed ? (1 - value) * (content.Area.SeriesClipRect.Width - plotOffset) + chartAxis.GetActualPlotOffsetEnd() + content.InsidePadding :
                    value * (content.Area.SeriesClipRect.Width - plotOffset) + chartAxis.GetActualPlotOffsetStart()  + content.InsidePadding;
        ...
        private void ArragneRectBasedOnOrigin(ChartAxis element, int elementRowIndex, int actualRowIndex, double axisTop, double areaWidth, double axisHeight, ref bool isContinue)
        {
            ...
            if (chartAxis != null)
            {
                ...
                if (0 < value && 1 > value)
                {
                    ...
                    {
                        double position = (element.HeaderPosition == AxisHeaderPosition.Far) ? (1 - value) * (rect.Width - plotOffset) + chartAxis.GetActualPlotOffsetStart() + element.InsidePadding : 0;
                        if (position > 0 && position > desiredSize.Width - element.headerContent.DesiredSize.Height)
                            element.ArrangeRect = new Rect(
                                ((rect.Width - plotOffset) * value)
                                - element.InsidePadding + chartAxis.GetActualPlotOffsetStart(),
                                axisTop,
                                desiredSize.Width, 
                                axisHeight);
                        else
                    ...
                    {
                        double position = (element.HeaderPosition == AxisHeaderPosition.Far) ? value * (rect.Width - plotOffset) + chartAxis.GetActualPlotOffsetStart() + element.InsidePadding : 0;
                        if (position > 0 && position > desiredSize.Width - element.headerContent.DesiredSize.Height)
                            element.ArrangeRect =
                           new Rect(
