...
namespace Syncfusion.UI.Xaml.Charts
{
    ...
    internal class ChartColumnDefinition : DependencyObject
    {
        ...
        internal void MeasureLegends(Size size, List<double> nearSizes, List<double> farSizes)
        {
            ...
            {
                if (content != null && content.DockPosition != ChartDock.Floating && content.GetPosition() == LegendPosition.Outside)
                {
            ...
            List<double> farSizes)
        {
            ...
            for (int i = 0; i < Legends.Count; i++)
            {
                ...
                if (element != null && element.DockPosition != ChartDock.Floating && element.GetPosition() == LegendPosition.Outside)
                {
        ...
        private void MeasureRectBasedOnOrigin(ChartAxis content, List<double> nearSizes, List<double> farSizes, ref bool isContinue)
        {
            ...
            if (0 < value && 1 > value)
            {
                ...
                {
                    double plotOffset = chartAxis.GetActualPlotOffset();
                    double position = content.OpposedPosition ? (1 - value) * (content.Area.SeriesClipRect.Height - plotOffset) + chartAxis.GetActualPlotOffsetStart() + content.InsidePadding :
                                      value * (content.Area.SeriesClipRect.Height - plotOffset) + chartAxis.GetActualPlotOffsetEnd() + content.InsidePadding;
            ...
            if (chartAxis != null)
            {
                ...
                if (0 < value && 1 > value)
                {
                    ...
                    if (element.OpposedPosition && elementColumnIndex == actualColumnIndex)
                    {
                        ...
                        double position = (element.HeaderPosition == AxisHeaderPosition.Far) ? (1 - value) * (rect.Height - plotOffset) + chartAxis.GetActualPlotOffsetEnd() + element.InsidePadding : 0;
                        if (position > 0 && position > desiredSize.Height - element.headerContent.DesiredSize.Height)
                            ...
                            element.ArrangeRect = new Rect(
                                ...
                                desiredSize.Height) +
                                element.InsidePadding + chartAxis.GetActualPlotOffsetEnd(),
                                axisWidth, 
                                desiredSize.Height);
                    }
                    else if (elementColumnIndex == actualColumnIndex)
                    {
                        double position = (element.HeaderPosition == AxisHeaderPosition.Far) ? value * (rect.Height - plotOffset) + chartAxis.GetActualPlotOffsetEnd() + element.InsidePadding : 0;
                        if (position > 0 && position > desiredSize.Height - element.headerContent.DesiredSize.Height)
                            element.ArrangeRect = new Rect(
                                actualLeft,
