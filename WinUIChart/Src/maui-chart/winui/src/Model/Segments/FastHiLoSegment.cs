...
namespace Syncfusion.UI.Xaml.Charts
{
    ...
    internal class FastHiLoSegment : ChartSegment
    {
        #region Fields
            ...
            if (Series.DataCount > 0)
            {
                ...
                if (Series.IsIndexed)
                {
                    double X_MAX = ((Series.ActualXAxis is CategoryAxis && !(Series.ActualXAxis as CategoryAxis).IsIndexed)) ?
                     xChartVals.Max() : Series.DataCount - 1;
        ...
        {
            var fastHiloSeries = Series as ChartSeries;
            ...
            if (bitmap != null && xValues.Count > 0)
            {
                ...
                {
                    ChartBase chart = fastHiloSeries.Area;
                    ...
                    {
                        Brush segmentSelectionBrush = (fastHiloSeries as ISegmentSelectable).SelectionBrush;
        ...
        private void UpdateVisualVertical(int width, int height,Windows.UI.Color color, int leftThickness, int rightThickness, bool isMultiColor, int dataCount)
        {
            var fastHiloSeries = Series as ChartSeries;
            float xStart = 0;
            float yStart = 0;
            float xEnd = 0;
            float yEnd = 0;
            Windows.UI.Color segmentColor = color;
            ...
            for (int i = 0; i < dataCount; i++)
            {
                ...
                else if (fastHiloSeries.SelectedSegmentsIndexes.Contains(startIndex) && (fastHiloSeries as ISegmentSelectable).SelectionBrush != null)
        ...
        private void UpdateVisualHorizontal(int width, int height, Windows.UI.Color color, int leftThickness, int rightThickness, bool isMultiColor, int dataCount)
        {
            var fastHiloSeries = Series as ChartSeries;
            float xStart = 0;
            float yStart = 0;
