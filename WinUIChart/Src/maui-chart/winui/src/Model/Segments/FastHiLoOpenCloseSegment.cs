...
namespace Syncfusion.UI.Xaml.Charts
{
    ...
    internal class FastHiLoOpenCloseSegment : ChartSegment
    {
        #region Fields
        ...
        {
            bitmap = (Series as AdornmentSeries).Area.GetFastRenderSurface();
            fastBuffer = (Series as AdornmentSeries).Area.GetFastBuffer();
        }
        ...
        {
            var fastHiLoOpenCloseSeries = Series as AdornmentSeries;
        ...
        {
            DoubleRange sbsInfo = Series.GetSideBySideInfo(Series);
            ...
            if (Series.DataCount > 0)
            {
                ...
                if (Series.IsIndexed)
                {
                    X_MAX = ((Series.ActualXAxis is CategoryAxis && !(Series.ActualXAxis as CategoryAxis).IsIndexed)) ?
                       Series.GroupedXValuesIndexes.Max() : Series.DataCount - 1;
                    X_MIN = 0;
                }
                else
                {
        ...
        internal void UpdateVisual(bool updateHiLoLine)
        {
            ...
            if (bitmap != null && xValues.Count > 0)
            {
                ...
                {
                    ChartBase chart = fastHiLoOpenCloseSeries.Area;
                    ...
                    {
                        Brush segmentSelectionBrush = (fastHiLoOpenCloseSeries as ISegmentSelectable).SelectionBrush;
        ...
        private void UpdateVisualVertical(int width, int height, int leftThickness, int rightThickness)
        {
            ...
            for (int i = 0; i < xValues.Count; i++)
            {
                ...
                else if (fastHiLoOpenCloseSeries.SelectedSegmentsIndexes.Contains(startIndex) && (fastHiLoOpenCloseSeries as ISegmentSelectable).SelectionBrush != null)
