        ...
        public class ChartCartesianTransformer : IChartTransformer
        {
            ...
            public Point TransformToVisible(double x, double y, bool isXInversed, bool isYInversed)
            {
                ...
                if (this.m_IsRoated)
                {
                    ...
                    return new Point(left + YAxis.RenderedRect.Width * YAxis.ValueToCoefficient(y, isYInversed), top + XAxis.RenderedRect.Height * (1 - XAxis.ValueToCoefficient(x, isXInversed)));
                }
                else
                {
                    double left = XAxis.RenderedRect.Left - XAxis.Area.SeriesClipRect.Left;
                    double top = YAxis.RenderedRect.Top - YAxis.Area.SeriesClipRect.Top;
                    return new Point(left + Math.Round(XAxis.RenderedRect.Width * XAxis.ValueToCoefficient(x, isXInversed)), top + Math.Round(YAxis.RenderedRect.Height * (1 - YAxis.ValueToCoefficient(y, isYInversed))));
                }
            }
