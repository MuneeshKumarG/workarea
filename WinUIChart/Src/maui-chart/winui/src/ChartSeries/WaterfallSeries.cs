...
namespace Syncfusion.UI.Xaml.Charts
{
    ...
    internal class WaterfallSeries : XyDataSeries, ISegmentSelectable, ISegmentSpacing
    {
            ...
            if (xValues != null)
            {
                ...
                for (int i = 0; i < this.DataCount; i++)
                {
                    if (i < this.DataCount)
                    {
                        ...
                        {
                            segment = CreateSegment() as WaterfallSegment;
                            if (segment != null)
                            {
                                segment.XData = xValues[i];
                                segment.YData = YValues[i];
            ...
            foreach (ChartSegment segment in Segments)
            {
                ...
                if (!double.IsNaN(elementHeight) && !double.IsNaN(YValues[i]))
                {
                    ...
                    if (this.AdornmentsInfo != null && AdornmentsInfo.Visible)
                    {
                        ...
                        keyFrames1.Duration = new Duration().GetDuration(TimeSpan.FromSeconds((AnimationDuration.TotalSeconds * 20) / 100));
        ...
        {
            WaterfallSegment waterfallSegment = ToolTipTag as WaterfallSegment;
            Point newPosition = new Point();

            if (waterfallSegment != null)
            {
                Point point = ChartTransformer.TransformToVisible(waterfallSegment.XData, waterfallSegment.Top);
                newPosition.X = point.X + ActualArea.SeriesClipRect.Left;
                newPosition.Y = point.Y + ActualArea.SeriesClipRect.Top;
            }
            else
            {
                ChartDataPointInfo dataPointInfo = ToolTipTag as ChartDataPointInfo;
                Point point = ChartTransformer.TransformToVisible(dataPointInfo.XData, dataPointInfo.YData);
                newPosition.X = point.X + ActualArea.SeriesClipRect.Left;
                newPosition.Y = point.Y + ActualArea.SeriesClipRect.Top;
            }
        ...
        private void GetSummaryValues()
        {
            ...
            else
            {
                ...
                if (dataTable.Columns.Contains(SummaryBindingPath))
                {
                    ...
                    if (enumerator.MoveNext())
                    {
                        do
                        {
                            object pathValue = (enumerator.Current as DataRow)[SummaryBindingPath];
                            summaryValues.Add((bool)pathValue);
                        }
                        while (enumerator.MoveNext());
                    }
                }
    ...
    /// </summary>
    internal class WaterfallSegmentCreatedEventArgs : EventArgs
    {
        #region Public
