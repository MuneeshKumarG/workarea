    ...
    public class ScatterSeries : XySegmentDraggingBase, ISegmentSelectable
    {
        ...
        internal override void ActivateDragging(Point mousePos, object element)
        {
            try
            {
                ...
                if (selectionBehavior != null && selectionBehavior.ActivationMode == SelectionActivationMode.Click)
...
#region Private Methods
        ...
        private void UpdatePreviewIndicatorPosition(Point mousePos, FrameworkElement frameworkElement)
        {
            if (previewElement == null)
            {
                ...
                {
                    if (AdornmentsInfo.MarkerType.ToString() == "Custom")
                        AnimateAdornmentSymbolTemplate(positionX, positionY);
                    else
                    {
                        AddAnimationEllipse(
                            ChartDictionaries.GenericSymbolDictionary["Animation" + AdornmentsInfo.MarkerType.ToString()] as ControlTemplate,
                            AdornmentsInfo.MarkerHeight,
                            AdornmentsInfo.MarkerWidth,
                            positionX,
                            positionY,
        ...
        private void SegmentPreview(Point mousePos)
        {
            try
            {
                ...
                DraggedXValue = (DragDirection == DragType.Y || IsIndexed) ?
                    ...
                    Area.ActualPointToValue(ActualXAxis, mousePos);

                DraggedValue = (DragDirection == DragType.X) ?
                    draggingSegment.YData :
                    Area.ActualPointToValue(ActualYAxis, mousePos);
