...
namespace Syncfusion.UI.Xaml.Charts
{
    internal class CategoryAxisHelper
    {
        ...
        internal static object GetLabelContent(ChartAxis axis, double position)
        {
            ...
            {
                if (axis.CustomLabels.Count > 0 || axis.GetLabelSource() != null)
                {
        ...
        internal static object GetLabelContent(ChartAxis axis, int pos, ChartSeriesBase actualSeries)
        {
            ...
            if (actualSeries != null)
            {
                ...
                foreach (ChartSeriesBase chartSeries in axis.RegisteredSeries)
                {
                    IEnumerable pointValues = chartSeries.ActualXValues;
                    ChartValueType valueType = chartSeries.XAxisValueType;
                    label = string.Empty;
                    ...
                    if (values != null && pos < values.Count && pos >= 0)
                    {
                        ...
                        if (valueType == ChartValueType.DateTime)
                        {
                            ...
                            label = xDateTime.ToString(axis.LabelFormat, CultureInfo.CurrentCulture);
                        }
                        else if (valueType == ChartValueType.TimeSpan)
                        {
                            TimeSpan xTimeSpanValue = TimeSpan.FromMilliseconds(values[pos]);
                            label = xTimeSpanValue.ToString(axis.LabelFormat, CultureInfo.CurrentCulture);
                        }

                        else if (valueType == ChartValueType.Double || valueType == ChartValueType.Logarithmic)
                        {
                            label = values[pos].ToString(axis.LabelFormat, CultureInfo.CurrentCulture);
                        }
                    ...
                    else
                    {
                        ...
                        if (StrValues != null && pos < StrValues.Count && pos >= 0)
                        {
                            if (!String.IsNullOrEmpty(axis.LabelFormat))
                                label = String.Format(axis.LabelFormat, StrValues[pos]);
                            label = StrValues[pos];
                        }
                    }
                    ...
                    if ((chartSeries is WaterfallSeries || chartSeries is HistogramSeries || chartSeries is PolarRadarSeriesBase || chartSeries is ErrorBarSeries))
                    {
                        return labelContent;
                    }
        ...
        internal static void GenerateVisibleLabels(ChartAxis axis, LabelPlacement labelPlacement)
        {
            ...
            actualSeries = actualSeries != null ? actualSeries : axis.Area is ChartBase ? (axis.Area as ChartBase).TechnicalIndicators.Where(series => series.ActualXAxis == axis)
            ...
            if (isPolarRadarSeries || labelPlacement == LabelPlacement.OnTicks) return;

            if (position == 0 && axis.VisibleRange.Inside(-0.5))
            {
                axis.m_smalltickPoints.Add(-0.5);
                position = -0.5 + interval;
            }
            ...
            for (; position <= visibleRange.End; position += interval)
            {
                ...
                {
                    axis.m_smalltickPoints.Add(position);
                }
            }
        }
