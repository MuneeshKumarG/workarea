...
namespace Syncfusion.UI.Xaml.Charts
{
    ...
    internal class MultiLevelLabelsPanel
    {
        #region Fields
        ...
        private Thickness margin = new Thickness().GetThickness(2, 2, 2, 2);
...
#region Internal Methods
        ...
        internal Size Arrange(Size finalSize)
        {
            ...
            if (Axis.Orientation == Orientation.Horizontal)
            {
                ...
                rect = new Rect(
                    Axis.GetActualPlotOffsetStart() - maxThickness,
                    top,
                    (finalSize.Width + 2 * maxThickness - Axis.GetActualPlotOffset()),
            ...
            else
            {
                ...
                rect = new Rect(
                    left,
                    Axis.GetActualPlotOffsetEnd() - maxThickness,
                    finalSize.Width + margin.Left + margin.Right,
                    (finalSize.Height + 2 * maxThickness - Axis.GetActualPlotOffset()));
...
#region Private Methods
        ...
        private bool SetValues(ChartMultiLevelLabel label)
        {
            ...
            if (Axis.Orientation == Orientation.Horizontal)
            {
                start = Math.Round(Axis.ValueToCoefficient(startValue)
                    * (Axis.RenderedRect.Width)) + Axis.GetActualPlotOffsetStart();
            ...
            else
            {
                start = (1 - Axis.ValueToCoefficientCalc(startValue)) *
                     Axis.RenderedRect.Height + Axis.GetActualPlotOffsetEnd();
        ...
        private void SetVerticalLabelRectangle(int row)
        {
            ...
            if (row == 0 && Axis.ShowLabelBorder)
                ...
                new Thickness().GetThickness(0, thickness, thickness, thickness) :
                new Thickness().GetThickness(thickness, thickness, 0, thickness);
            else
                border.BorderThickness = new Thickness().GetThickness(thickness,thickness,thickness,thickness);
        ...
        private void SetVerticalBorderWithoutTopAndBottom()
        {
            var thickness = Axis.LabelBorderWidth;
            var border = borderRecycler[currentBorderPos];
            border.BorderThickness = new Thickness().GetThickness(0, thickness, 0, thickness);
        ...
        private void SetBorderPosition()
        {
            ...
            if (Axis.MultiLevelLabelsBorderType == BorderType.None)
                border.BorderThickness = new Thickness().GetThickness(0, 0, 0, 0);
            if (Axis.Orientation == Orientation.Horizontal)
                SetHorizontalBorder(border);
            else
                SetVerticalBorder(border);
        }
        ...
        private void SetVerticalLabelAlignment(TextBlock txtBlock)
        {
            ...
            if (Axis.OpposedPosition)
                xPos = Axis.GetLabelPosition() == AxisElementPosition.Outside ? (left - borderPadding) + Axis.LabelBorderWidth / 2 :
        ...
        private void CheckLabelRange()
        {
            ...
            var textBlock = textBlockRecycler.Where(txtBlock => txtBlock.Tag as ChartMultiLevelLabel == currentLabel).First();
        ...
        private void SetBorderStyle(int row, bool isHorizontalAxis)
        {
            ...
            else
            {
                ...
                {
                    if (isHorizontalAxis)
                        SetHorizontalLabelRectangle(row);
                    else
                        SetVerticalLabelRectangle(row);
                }
                else
                {
                    if (isHorizontalAxis)
                        SetHorizontalBorderWithoutTopandBottom();
                    else
                        SetVerticalBorderWithoutTopAndBottom();
                }
        ...
        private void SetHorizontalLabelRectangle(int row)
        {
            ...
            if (row == 0 && Axis.ShowLabelBorder)
                border.BorderThickness = isOpposed ?
                    new Thickness().GetThickness(thickness, thickness, thickness, 0) :
                    new Thickness().GetThickness(thickness, 0, thickness, thickness);
            else
                border.BorderThickness = new Thickness().GetThickness(thickness, thickness, thickness, thickness);
...
#else
            border.BorderThickness =ThicknessHelper.FromLengths(thickness, 0, thickness, 0);
#endif
            SetBorderPosition();
        }
        ...
        private void SetHorizontalBrace()
        {
            ...
            var textBlock = textBlockRecycler.Where(child => child.Tag as ChartMultiLevelLabel == currentLabel).FirstOrDefault();
            if (textBlock != null)
            {
        ...
        private void SetHorizontalLabelAlignment(TextBlock txtBlock)
        {
            switch (currentLabel.LabelAlignment)
            {
                ...
                case LabelAlignment.Center:
                    {
                        var midValue = startValue + ((endValue - startValue) / 2);
                        txtBlockLeft = Axis.GetActualPlotOffsetStart() +
        ...
        private bool IsOpposed()
        {
            return ((Axis.OpposedPosition)
                && Axis.GetLabelPosition() == AxisElementPosition.Outside)
                 || (!(Axis.OpposedPosition)
                 && Axis.GetLabelPosition() == AxisElementPosition.Inside);
        }
