    ...
    public class ChartCartesianGridLinesPanel : ILayoutCalculator
    {
        ...
        private void UpdateHorizontalStripLine(ChartAxisBase2D axis)
        {
            ...
            foreach (var stripLine in axis.StripLines)
            {
                ...
                ChartAxis cusAxis = Area.InternalAxes[stripLine.SegmentAxisName];
                if (cusAxis != null)
                {
                    ...
                    if (count > 0)
                    {
                        ...
                        if (!stripLine.IsSegmented)
                        {
                            ...
                            do
                            {
                                ...
                                double x1 = Area.ActualValueToPoint(axis, startStrip);
                                ...
                                {
                                    x2 = Area.ActualValueToPoint(axis, stripEnd);
                                }
                                axis.StriplineXRange += new DoubleRange(stripStart, Area.ActualPointToValue(axis, new Point(x2, 0)));
                        ...
                        else
                        {
                                ...
                                !double.IsNaN(stripLine.SegmentEndValue))
                            {
                                do
                                {
                                    ...
                                    axis.StriplineXRange += new DoubleRange(startStrip, Area.ActualPointToValue(axis, new Point(x2, 0)));
                                    axis.StriplineXRange += currAxis.StriplineXRange;
                                    if (axis.Area != null)
                                    {
                                        double startVal = Area.ActualValueToPoint(currAxis,
                                            stripLine.SegmentStartValue);
                                        double endVal = Area.ActualValueToPoint(currAxis,
                ...
                foreach (var xAxis in xAxes)
                {
                    ...
                    if (count > 0)
                    {
                        ...
                        if (!stripLine.IsSegmented)
                        {
                            ...
                            do
                            {
                                double y1 = Area.ActualValueToPoint(axis, startStrip);
                                double y2;
                                if (stripLine.IsPixelWidth)
                                {
                                ...
                                else
                                {
                                    double stripEnd = (!double.IsNaN(stripLine.RepeatUntil) ? (startStrip < stripLine.RepeatUntil) ? startStrip + stripLine.Width
                                            : startStrip - stripLine.Width : startStrip + stripLine.Width);
                                    y2 = Area.ActualValueToPoint(axis, stripEnd);
                                }
                                axis.StriplineYRange += new DoubleRange(startStrip, Area.ActualPointToValue(axis, new Point(0, y2)));
                        ...
                        else
                        {
                                ...
                                !double.IsNaN(stripLine.SegmentEndValue))
                            {
                                do
                                {
                                    ...
                                    {
                                        double startVal = Area.ActualValueToPoint(xAxis,
                                            stripLine.SegmentStartValue);
                                        double endVal = Area.ActualValueToPoint(xAxis,
                                            stripLine.SegmentEndValue);
