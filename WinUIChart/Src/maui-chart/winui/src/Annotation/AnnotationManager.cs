...
namespace Syncfusion.UI.Xaml.Charts
{
    ...
    internal class AnnotationManager : DependencyObject
    {
        ...
        internal ChartBase Chart
        {
            ...
            set
            {
                if (chart != null)
                {
                    ...
                    chart.InternalAxes.CollectionChanged -= OnAxesCollectionChanged;
        ...
        private bool OnEnableDrag(Point point)
        {
            if (SelectedAnnotation != null)
            {
                if (selectedAnnotation.CoordinateUnit == CoordinateUnit.Axis && selectedAnnotation.EnableClipping)
                {
                    ...
                    double xval = this.Chart.ActualPointToValue(selectedAnnotation.XAxis, new Point(point.X - left, point.Y - top));
                    double yval = this.Chart.ActualPointToValue(selectedAnnotation.YAxis, new Point(point.X - left, point.Y - top));
        ...
        private void OnManipulationDelta(object sender, ManipulationDeltaRoutedEventArgs e)
        {
            ...
            {
                if (SelectedAnnotation.XAxis != null && SelectedAnnotation.XAxis.Orientation == Orientation.Vertical && SelectedAnnotation.CoordinateUnit == CoordinateUnit.Axis)
        ...
        private Point GetToolTipPosition(Point currentPosition, Annotation annotation)
        {
            ...
            switch (annotation.ToolTipPlacement)
            {
                ...
                case ToolTipLabelPlacement.Right:
                    Tooltip.Margin = new Thickness().GetThickness(10, 0, 0, 0);
                    break;
        ...
        private void AnnotationDrag(double xTranslate, double yTranslate)
        {
            ...
            if (selectedAnnotation.CanDrag && isDraggable)
            {
                ...
                else
                {
                    ...
                    {
                        x2 = (isAll || isYAxis) ? Annotation.ConvertToObject(CalculatePointValue(selectedAnnotation.X2, -xAxisChange, false, false), selectedAnnotation.XAxis) : selectedAnnotation.X2;
                        x1 = (isAll || isYAxis) ? Annotation.ConvertToObject(CalculatePointValue(selectedAnnotation.X1, -xAxisChange, false, false), selectedAnnotation.XAxis) : selectedAnnotation.X1;
                        y2 = (isAll || isXAxis) ? Annotation.ConvertToObject(CalculatePointValue(selectedAnnotation.Y2, -yAxisChange, true, false), selectedAnnotation.YAxis) : selectedAnnotation.Y2;
                        y1 = (isAll || isXAxis) ? Annotation.ConvertToObject(CalculatePointValue(selectedAnnotation.Y1, -yAxisChange, true, false), selectedAnnotation.YAxis) : selectedAnnotation.Y1;
                    }
                }
        ...
        private void OnAxisVisibleRangeChanged(object sender, VisibleRangeChangedEventArgs e)
        {
            ...
            if (AnnotationResizer != null && (AnnotationResizer.XAxis == chartAxis || AnnotationResizer.YAxis == chartAxis))
            {
            ...
            if (this.chart != null && this.chart.ChartAnnotationCanvas != null)
            {
                switch (e.Action)
                {
                    ...
                    case NotifyCollectionChangedAction.Reset:
                        var chartChildren = chart.ChartAnnotationCanvas.Children; // WPF-57246-Tooltips disappear after clearing a charts Annotation collection

                        if (chartChildren.Count > 0)
                        {
                            var chartChildAnnotation = new List<UIElement>(); 
                            foreach (UIElement child in chartChildren)
                            {
                                chartChildAnnotation.Add(child);
                            }

                            foreach (var child in chartChildAnnotation)
                            {
                                var annotation = child as Annotation;
                                if (annotation != null)
                                {
                                    var annotationElement = annotation.GetRenderedAnnotation();
                                    if (this.chart.ChartAnnotationCanvas.Children.Contains(annotationElement))
                                        RemoveChartAnnotation(annotation, annotationElement);
                                }
                            }
                        ...
                        var seriesChildren = chart.SeriesAnnotationCanvas.Children;

                        if (seriesChildren.Count > 0)
                        {
                            var seriesChildAnnotation = new List<UIElement>(); 
                            foreach (UIElement child in seriesChildren)
                            {
                                seriesChildAnnotation.Add(child);
                            }

                            foreach (var child in seriesChildAnnotation)
                            {
                                var annotation = child as Annotation;
                                if (annotation != null)
                                {
