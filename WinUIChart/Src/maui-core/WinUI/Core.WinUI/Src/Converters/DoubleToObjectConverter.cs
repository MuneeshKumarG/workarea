...
namespace Syncfusion.UI.Xaml.Core
{
    ...
    public class DoubleToObjectConverter : DependencyObject, IValueConverter
    {
        ...
        public object Convert(object value, Type targetType, object parameter, string language)
        {
            ...
            if (!double.IsNaN(this.GreaterThan) && !double.IsNaN(this.LessThan) &&
                vd > this.GreaterThan && vd < this.LessThan)
            {
                boolValue = true;
            }
            else if (!double.IsNaN(this.GreaterThan) && vd > this.GreaterThan)
            {
                boolValue = true;
            }
            else if (!double.IsNaN(this.LessThan) && vd < this.LessThan)
            {
                boolValue = true;
            }
