...
namespace Syncfusion.UI.Xaml.Charts
{
    ...
    public class SplineAreaSegment : AreaSegment
    {
        ...
        {
            var containerSeries = Series as SplineAreaSeries;
            if (isSegmentUpdated)
            ...
            double origin = 0;//setting origin value for splinearea segment
                ...
                if (xVal >= range.Start && xVal <= range.End || xEnd >= range.Start && xEnd <= range.End)
                {
                    ...
                    else
                    {
                        ...
                        else if (i > 0 && (double.IsNaN(segmentPoints[i - 1].Y) || double.IsNaN(segmentPoints[i].Y)))
                        {
                            ...
                            if ((!double.IsNaN(segmentPoints[i + 2].Y)) || !containerSeries.IsClosed)
                            {
                                strokeFigure = new PathFigure();
